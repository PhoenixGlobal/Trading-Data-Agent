...
def get_address_token(address: str, chain_name: str,token_contract_address: str):
    """Query the holdings of a specific blockchain address for a specific token. If only the token symbol is provided without the contract address, first retrieve the contract address based on the symbol. Return the token symbol, contract address, balance, token price, and token value (in USD).
